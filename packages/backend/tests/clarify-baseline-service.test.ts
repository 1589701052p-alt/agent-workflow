...
describe('RFC-058 baseline T2 — buildClarifyPromptContext multi-round + inline', () => {
  ...
  // RFC-070: the legacy `historyCutoffClarifyIteration` filter is gone —
  // aging is now per-row via `consumed_by_consumer_run_id IS NULL`. The
  // semantic equivalent ("drop rounds already baked into a prior done run")
  // is verified by rfc070-aging-stamp-behavior.test.ts B-group cases.
