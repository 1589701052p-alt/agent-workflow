...
Live LLM tests are **opt-in**. They run only when BOTH conditions hold at
...
To override the opencode binary path (e.g. test a not-yet-installed
release):

```sh
OPENCODE_BIN=/opt/opencode-1.16.0/bin/opencode \
...
and on PRs that touch the daemon's opencode-facing code (path filter:
...
| OS            | opencode                                                    |
| ------------- | ----------------------------------------------------------- |
| ubuntu-latest | pinned `opencode-ai@1.15.5` (MIN_OPENCODE_VERSION baseline) |
| ubuntu-latest | `opencode-ai@latest` (drift signal)                         |

Both legs use the same secrets — see "Setting up CI secrets" below.
...
| Matrix × 2 OS legs | no — ubuntu only    | macOS runners are slow + expensive; cross-OS opencode parity is already covered by the daemon's PR e2e                    |

Expected daily LLM cost: ~10 calls × ~$0.005 = ~$0.05/day = ~$18/year.
...
2. **Check the opencode version** — `OPENCODE_BIN --version`. The two
   matrix legs intentionally pin different versions; one going red while
   the other stays green narrows the drift.
...
4. **Don't disable** — these tests guard the daemon's runtime contract.
   If opencode genuinely broke compat, pin the working version in
   `MIN_OPENCODE_VERSION` and open an issue upstream. Don't skip.
