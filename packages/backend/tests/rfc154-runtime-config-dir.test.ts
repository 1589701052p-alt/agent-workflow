...
describe('RFC-154 stageSkills', () => {
  ...
  test('managed → copy, external → symlink, project → skip, missing sourcePath → skip+warn', () => {
    ...
    try {
      ...
      // external skill: POSIX -> dir symlink (isSymbolicLink); Windows -> junction
      // via linkSkillDir (a mount-point reparse, NOT reported as isSymbolicLink).
      const externalLink = join(cfg, 'skills', 'e')
      if (process.platform === 'win32') {
        expect(existsSync(externalLink)).toBe(true)
        expect(realpathSync(externalLink)).toBe(realpathSync(externalSrc))
      } else {
        expect(lstatSync(externalLink).isSymbolicLink()).toBe(true)
      }
