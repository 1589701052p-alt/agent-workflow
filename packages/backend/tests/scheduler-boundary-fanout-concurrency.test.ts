...
describe('regression: wrapper-fanout shard dispatch MUST honor concurrency caps (scheduler.ts:2496 dead subprocessSem)', () => {
  ...
  test('B: non-readonly (writer) shards serialize via writeSem even when caps allow parallelism', async () => {
    ...
    const t = (await h.db.select().from(tasks).where(eq(tasks.id, taskId)))[0]
    expect(t?.status).toBe('done')
    // Writers on one worktree must serialize: 3 x 400ms ≈ 1200ms+.
    expect(elapsed).toBeGreaterThan(1000)
  }, 30_000)
})
