...
export function useTaskSync(taskId: string | null): void {
  ...
  const clarifyKeys = (nodeRunId: string): QueryKey[] => [
    ...
    // answer item; a cross-clarify answer adds a clarify_answer item. All timeline.
    ['task-timeline', taskId],
  ]
  ...
  const rules: WsInvalidationRules<TaskWsMessage> = {
    ...
    'node.status': () => [
      ...
      // RFC-W002: a run flipping to done surfaces a new node_output timeline item
      // (the feed aggregates node_run_outputs). node.event is intentionally NOT
      // invalidated - it fires per opencode event (per token) and the feed only
      // changes on the done transition, which this branch already covers.
      ['task-timeline', taskId],
    ],
