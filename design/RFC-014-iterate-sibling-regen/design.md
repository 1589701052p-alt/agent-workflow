...
### 3.2 后端渲染源（v2 修订：只贴路径不贴正文）

`services/review.ts` 新增 `buildSiblingOutputsBlock(args: { db, appHome, taskId, upstreamNodeId, targetPortName }): Promise<string | undefined>`：

1. 对每个 sibling port，查询该 task + 该上游节点的最新 `doc_versions` 行（按 `reviewIteration desc, createdAt desc` 取第一行）。
2. 读 `doc_versions.sourceFilePath`——只对 `kind = 'markdown_file'` 端口非空（agent 用 envelope 报告了写到哪个 worktree-相对文件路径）。`kind = 'markdown'` 内联端口 `sourceFilePath = null/空串`，**直接跳过不入列表**。
3. 拼接：
...
- {portA_name}: design/proposal.md
- {portB_name}: design/plan.md
```

4. 任一 sibling 没有任何 doc_version → skip；sibling 有 doc_version 但 sourceFilePath 为空（inline markdown）→ skip；全 0 → 返 `undefined`，上游 caller 走"等同 RFC-005 现行 iterate"老路径。

**为什么不贴正文**：agent 重跑时 cwd 就是 worktree，可自行 re-read sibling 文件；只贴路径让 prompt 保持紧凑、避免 worktree 已被外部改动时注入陈旧快照。代价：纯 inline markdown 上游的 sibling 同步引导能力为零（这类 agent 没有 worktree 文件，agent 也无法 re-read）；产品层接受这个取舍，鼓励多文档场景下用 `kind: markdown_file`。

英文前缀是稳定契约——`tests/review-prompt-injection.test.ts` + `review-iterate-sibling-cascade.test.ts` 断言字面量，不许后续无声改写。
...
- `review-sibling-outputs-prompt.test.ts`（C2）
  - case 1（v2）：渲染产物含英文前缀字面量 + 每个有 sourceFilePath 的 sibling 一行 `- {port}: {worktree path}`；无 sourceFilePath 的 inline 端口跳过。
