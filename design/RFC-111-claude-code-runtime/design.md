...
2026-06-27 **实现 gate**（codex-cli read-only，范围限定 RFC-111 实现文件、排除工作树协作者 RFC-108）。verdict=needs-fix，**5 findings 全部修复 + 各带回归测试**：

| # | 级别 | finding | 修复 |
|---|---|---|---|
| 1 | P1 | claude 分支复用 `opts.opencodeCmd`（生产经 `resolveOpencodeCmd(config)` 从 opencodePath 设），自定义 opencodePath 会让 claude spawn opencode 二进制 + 跳过凭据桥接 | 新增 `RunNodeOptions.runtimeCmd`（测试专用 head 覆盖）；claude 用 `runtimeCmd`（生产→`['claude']`），桥接 gate 改 `runtimeCmd === undefined`；claude 测试改传 runtimeCmd。 |
| 2 | P1 | 缺 claude 二进制 → `Bun.spawn` ENOENT 抛出、node_run 卡 `running` | `runNode` wrap `Bun.spawn` try/catch → clean `failed` RunResult（`setNodeRunStatus` allowedFrom running/pending + `runtime-spawn-failed`），driver cleanup 调用。回归：缺二进制→failed 不抛、行不卡 running。 |
| 3 | P2 | fanout shard / aggregator 的 runNode 调用没接 `resolveFrozenRuntime`，claude-selected agent-multi 静默跑 opencode | scheduler 两处 fanout 调用加 `resolveFrozenRuntime` + `runtime` 冻结。 |
| 4 | P2 | 非法冻结 runtime 静默重解析（违 D15） | `resolveFrozenRuntime` 非 null 非法值加 loud `frozen-runtime-invalid-reresolved` warn（保留 re-resolve 作前向兼容恢复——与 runtime-freeze 测试一致，注明：session 配对仅在同时有 captured id 时有风险，re-resolve 走全新会话）。 |
| 5 | P3 | claude 非零退出报成 `opencode exited with code` | exit 错误消息参数化 `${runtime} exited with code`。 |

门禁：typecheck×3 + 全量 backend 4099 pass/0 fail + prettier。
