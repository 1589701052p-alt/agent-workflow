...
### 15.5 已知限制（窄、**非腐化**，立后续专项修 — 2026-06-29 用户拍板「先上线」同 RFC-122 残留）
**残留**：手动问题**创建**落在「调度器已 derive `done`、尚未执行 done-CAS」的窗口内、且任务是**运行中 deferred 任务** → stale done-CAS（`trySetTaskStatus(done, allowedFrom=['running'])`、状态仍 running）落库 → 手动行搁浅在 `done` 任务上。
**非腐化**：round-4 的 dispatch 终态 in-tx CAS 拒绝在 done 任务下发（无 stamp/mint）→ 搁浅手动行**绝不**产生 orphan node_run；残留仅一条**可见但不可下发**的 staged 手动问题。
**dispatch-first 变体不可达**：任何未下发 designer/manual 行都 park 任务（`awaiting_human`）→ 调度器有未下发行时绝不 derive `done` → 只能从 parked 态下发；唯一可达竞态是「新建手动行」落 done-derive 窗口（reassign 改既有行、已 park）。
**为何未当场修**：原子修需扩展 RFC-097 单一 s14 守卫 writer `setTaskStatus`（lifecycle.ts:253-293）的 CAS 加 WHERE 条件 + 把 JS 计算的 park gate（in-flight/consumed lineage + clarify_rounds join + manual union）翻 SQL（divergence 风险）+ deferred 门控（非 deferred 任务 lazy reconcile 合法留 `dispatched_at IS NULL` designer 行，ungated `NOT EXISTS` 会卡死非 deferred 完成 = golden-lock break）+ 新增 manual-create→scheduler kick 耦合（blocked done-CAS 后需 re-derive）。该组合对 heavily-tested 完成+lifecycle 的风险 > 窄非腐化残留（属 CLAUDE.md「比看上去更纠缠 → STOP green」）。
**后续专项修配方**（独立 PR、单独评审）：① `setTaskStatus` 加可选 `casExtraWhere`；done-CAS 传 `(deferred_question_dispatch=0 OR NOT EXISTS(SELECT 1 FROM task_questions WHERE task_id=tasks.id AND role_kind='designer' AND confirmation!='confirmed' AND dispatched_at IS NULL))` ② manual create 后 kick scheduler（blocked done 即 re-derive→`awaiting_human`）③ db Proxy 测（create 在 derive 后、done-CAS 前提交 → 任务落 awaiting_human、行可下发、无 orphan）。轻量替代（鉴于非腐化已足够）：manual create 后 best-effort kick（收窄窗口）+ 终态搁浅手动行的 UI 清理入口。
