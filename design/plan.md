...
| [RFC-032](./RFC-032-nav-redesign/proposal.md)                    | 前台导航重构：把 10 项平铺侧栏改成顶栏 4 个一级 tab（代理 / 工作流 / 任务 / 设置）+ 子导航条，技能 / MCP / 插件归入"代理"组（代理的能力），远端仓归入"任务"组（任务的执行环境）；评审 + 反问合并为顶栏右上"收件箱" chip + 浮窗 drawer（沿用 reviews/clarify pending-count 两端点，前端求和）；opencode 运行时升级为顶栏常驻 chip（绿/灰/红/黄 dot 映射 daemon 状态），点击跳 `/settings#runtime`；纯前端外壳重构，0 backend / 0 shared / 0 DB；新 `lib/nav.ts::resolveActiveNav` 纯函数 + 组件单测 + Playwright e2e 锁四条路径 | Draft       |
