...
| [RFC-035](./RFC-035-ux-consistency/proposal.md)                  | UX 一致性专项（**三件套 Draft 已展开**，待用户批准进入实现；背景见 `design/ux-audit.md` 全量审计）：PR1 引入 4 类设计 token（间距 `--space-1..6` / 字号 `--font-xs..xl` / 圆角 `--radius-sm..pill` / 阴影 `--shadow-sm..lg` + 语义色 `--success/--warn/--info`）+ 补 `.btn--ghost` `.btn--xs` 静默缺失 CSS（9 处 callsite，audit 当时 5 处 + RFC-032 PR2/PR3 新增 4 处）+ 新 `<StatusChip kind="success/warn/danger/info/neutral" size>` 共享组件 + retrofit 4 个分散组件（`TaskStatusChip` / `StatusBadge` / `McpProbeStatusChip` / homepage `task-row__status`）；PR2 `.tabs` 加 `--inline/--inspector/--segment` 三 modifier + retrofit `NodeDetailDrawer` / `NodeInspector` / `AgentImportDialog` / `RepoSourceTabs` 4 处 + `.data-table` 推广到 `repos` / `agent-import` / `reviews` 3 处 + `<Form>` helper 7 路由迁移（agents.new/detail + plugins.new/detail + mcps.new/detail + clarify.detail）；PR3 抽 `<Dialog>` `<EmptyState>` `<LoadingState>` `<DetailLayout>` 4 个共享组件 + retrofit `AgentImportDialog` / 内联 `ReviewDecisionDialog` / `BatchImportDialog` 3 处 dialog + 9 处 isLoading/isPending → EmptyState/LoadingState + task-detail / review-detail 套 `<DetailLayout>` + settings.tsx 6 处内联 style 清理；0 backend / 0 shared / 0 DB；旧命名空间 class 全部作 backward-compat alias 保留 30 天，最终 cleanup PR 独立提出（预估 -350~500 行 styles.css） | In Progress |
