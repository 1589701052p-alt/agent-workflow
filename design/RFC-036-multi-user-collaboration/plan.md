...
| PR  | Commit    | 状态 | 备注                                                                          |
| --- | --------- | ---- | ----------------------------------------------------------------------------- |
| PR0 | `78078dd` | Done | 三件套 Draft + STATE/plan 索引                                                |
| PR1 | `20dfe19` | Done | schema + 三轨鉴权 + bootstrap + CLI + 50 tests                                |
| PR2 | `c63dbc1` | Done | requirePermission gates + task visibility + 28 tests                          |
| PR3 | `418f6f2` | Done | OIDC providers + login flow + identity + 24 tests                             |
| PR4 | `af766a6` | Done | task collab + node assignments + reviewer/clarify gate + 10 tests             |
| PR5 | `ef7f5db` | Done | multi-user UI (login + /account + /users + UserMenu) + /api/users + 7 tests   |

**Follow-up（未覆盖范围，预计单独 PR 补齐）**：UserPicker 共享组件 + launcher
AssignmentsField 段 + `/settings/Authentication` tab UI（admin OIDC provider
CRUD 暂只能走 HTTP API） + `<NoPermissionEmpty>` 共享组件 + Playwright 多用户
e2e + 完整 130 i18n keys 中英对称（PR5 UI 现以 `defaultValue:` 兜底）。OIDC
end-to-end 真实 IdP smoke 留作 post-merge 手动验证（callback 流程 typed +
provisioning 6 path 单测覆盖，但无 mock IdP e2e）。
