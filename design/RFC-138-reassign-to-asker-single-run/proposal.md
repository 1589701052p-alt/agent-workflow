...
- **D6 与已下发的反问者续跑相遇 ⇒ 零新增 mint 是有意行为**（Codex 实现门 P1 裁决，落测试
  锁）：quick 路径答完即自动下发反问者批——续跑已带该题答案跑过/正在跑。此时 collapse 不再
  安排任何新 rerun，终态与「答题时就选 scope=questioner」完全同形；这正是本 RFC 要的「只跑
  一遍」（按 P1 建议补 mint = 复活双跑）。看板上该题的 questioner 卡照常走
  processing/awaiting_confirm→确认闭环，无悬挂。如确需提问节点**再**处理一遍，走手动问题。
