...
| session 状态 | `awaiting_human` | `awaiting_human`（复用；语义"等答案"，答者是人或 agent A 同构，UI 靠节点 kind 区分） |
...
5. **运行时核心数据流（方式一：A 重跑产回答回流 B）**：
   - B 跑完吐 `<workflow-clarify>` -> scheduler 检测 B.`__clarify__` 指向 to-agent 节点 -> 创建 to-agent session（`clarify_rounds` kind=`'to-agent'`）-> to-agent node_run mint，status=`awaiting_human`（复用 RFC-023 状态；等 A 回答，UI 靠节点 kind 区分）-> **触发 A 重跑**（rerun cause `clarify-to-agent-answer`）。
...
12. **新 node_run status：`awaiting_human`**：to-agent node_run mint 时 status=`awaiting_human`（等 A 回答）。加入 `shared/lifecycle.ts` 转移表：`pending -> awaiting_human`（mint）、`awaiting_human -> answered`（A 吐回答）、`awaiting_human -> awaiting_human`（A 升级问人，task 整体 park）、`awaiting_human -> canceled/failed`。RFC-053 invariants 加 1 条 to-agent 专用 invariant（awaiting_human session 的 answerer node_run 必须存在且非终态）。
...
15. **WS 事件**：`/ws/workflows` / per-task 加 3 个 event：
    - `clarify-to-agent.created`（B 提问、to-agent 进 awaiting_human、A 即将重跑）
    - `clarify-to-agent.answered`（A 吐回答、to-agent done、B 即将重跑）
    - `clarify-to-agent.escalated`（A 升级问人、to-agent 保持 awaiting_human、A 的 self-clarify park）
...
Launch task -> A 跑出 v1 产出 -> B 跑完读 A 产出，吐 `<workflow-clarify>` envelope 2 题（"这里为什么用 Redis？""限流是 P50 还是 P99？"）-> to-agent 节点进 `awaiting_human` -> 触发 A 重跑（cause=`clarify-to-agent-answer`）-> A prompt 含 `## Clarify Request` 段 + 2 题 + 协议指令 -> A 清楚 -> A 吐 `<workflow-clarify-answer>`（"Redis 因为 X / P99"）-> to-agent node_run done（`answered`）-> 回答回流 B.`__clarify_response__` -> B 重跑（cause=`clarify-to-agent-questioner-rerun`），B prompt 的 `## Clarify Q&A` 含 A 的回答 -> B 带回答产出 output -> task done。**全程无人介入。**

**S2（A 不清楚 -> 升级问人 -> A 答后答 B）**
同 S1 拓扑。B 反问 A 2 题 -> to-agent `awaiting_human` -> A 重跑看题 -> A 觉得其中 1 题自己也不清楚（"限流 P50 还是 P99 需求没写"）-> A 吐 `<workflow-clarify>` envelope（走 A 的 self-clarify 通道反问人）-> to-agent 保持 `awaiting_human`，A 的 self-clarify 节点 park awaiting_human -> task 顶层 awaiting_human。
...
to-agent `awaiting_human` -> A 重跑完只吐 `<workflow-output>`（A 忽略了 Clarify Request，直接产出）-> runner 检测 to-agent session 仍 `awaiting_human` 且 A 本次没吐 answer/clarify -> fail node_run `clarify-to-agent-timeout-no-answer`，retries 兜底。A 必须明确"答 B"或"问人"二选一。

**S7（多源：多个 B 反问同一个 A）**
工作流：`A -> B1 -> to-agent1(->A)` 和 `A -> B2 -> to-agent2(->A)`，两个 to-agent 都指向 A。B1、B2 各自反问 A -> 两个 to-agent session 都 `awaiting_human` -> A 被触发重跑（一次重跑 prompt 含两个 to-agent 的问题，按 nodeId 字典序拼 `## Clarify Request` 子段）-> A 一次回答 envelope 里作答两组问题 -> 两组回答回流各自的 B1/B2。**v1：A 一次重跑答所有指向它的 to-agent 问题**（多源汇总到一次 A 重跑，避免 A 被连续触发多次，对齐 RFC-056 多源汇总屏障设计）。
...
- **A4（A 升级问人）**：A 吐 `<workflow-clarify>` 走 self-clarify -> task awaiting_human -> 人答 A -> A 带答案重跑吐 answer -> 回流 B；**to-agent session 在 A 升级期间保持 `awaiting_human`**，A 的 self-clarify session 独立 `awaiting_human`。
...
- **A12（to-agent 状态：复用 awaiting_human）**：to-agent node_run mint 后 status=`awaiting_human`（复用 RFC-023 的 `park-human` 事件，不新增状态值）；A 吐 `<workflow-clarify-answer>` -> to-agent 转 `done`（`resume-clarify` 事件）；A 升级问人 -> to-agent 保持 `awaiting_human` + A 的 self-clarify 也 `awaiting_human`（task 整体 park）；A fail -> `failed`。UI 靠节点 kind（`clarify-to-agent`）区分 to-agent 与 self-clarify，不靠状态值。
- **A13（lifecycle invariant）**：RFC-053 invariant 扫描发现 `clarify_rounds` kind=`'to-agent'` 且 status=`awaiting_human` 的 session，其 answerer node_run 已终态（failed/canceled）-> 升级 session `abandoned` + warning。
...
- **A15（lifecycle 转移零新增）**：to-agent 复用 `park-human`（pending|running -> awaiting_human）+ `resume-clarify`（awaiting_human -> done）既有转移，不新增事件/状态。违反转移被 s14 守卫拒（既有行为）。
...
- **C3** `tests/clarify-to-agent-escalation.test.ts`：A 升级问人时 to-agent session 保持 `awaiting_human` + A 的 self-clarify 独立 `awaiting_human` + 人答后 A 答 B 全链。
- **C4** `tests/clarify-to-agent-multi-source.test.ts`：多 to-agent 指向同一 A，一次重跑答多组，分别回流。
- **C5** `tests/clarify-to-agent-lifecycle.test.ts`：to-agent 复用 `park-human`/`resume-clarify` 既有转移断言（无新状态可测矩阵）+ RFC-053 abandoned invariant。
...
1. **新 NodeKind vs 扩展 RFC-056 clarify-cross-agent**：选**新 NodeKind**。回答者不同（A vs 人）、回答时机不同（A 重跑即答 vs 人 POST 后 designer 重跑）、session 等待对象不同（`awaiting_human` vs `awaiting_human`）、A 的角色不同（回答生产者 vs 答案消费者）。强行给 cross-clarify 加 `answer-mode` 字段会把"人答路径"和"A 答路径"塞进同一节点，reject/多源汇总/abandoned 等 cross-clarify 语义对 A 答场景未必适用，单节点堆 mode 分支变胖。新 NodeKind 端口/校验/运行时路径独立，与 RFC-056 当年选新 NodeKind 的理由（proposal §5.1）同构。
...
5. **复用 `awaiting_human` vs 新增 `awaiting_answer` 状态**：选**复用 `awaiting_human`**（实现期修订，原设计拟新增 `awaiting_answer`）。`awaiting_human` 在 scheduler / stuckTaskDetector / workgroupRunner 有 ~20 触点，新增状态会触发全量扩散 + s12 status-bucket-universe 审计锁翻红；而 `awaiting_human` 天然覆盖"等答案"语义（人答 / agent A 答同构--都是等某种答案回填）。to-agent mint 用 `park-human`、A 答 -> `done` 用 `resume-clarify`，零新增事件/状态/转移。UI 区分靠节点 kind（`clarify-to-agent`），不靠状态值。代价：`awaiting_human` 字面是"等人"，在 to-agent 场景实为"等 agent A"，但语义抽象一致且 chip + 节点 kind 已足够区分。这是面向代码最合理的简化（记忆 `prefer-correct-over-minimal`）。
...
- **RFC-053 lifecycle hardening**：to-agent 复用 `awaiting_human` 状态（零新增状态）+ `to_agent_iteration` 列 + answerer node_run abandoned invariant 走 RFC-053 转移函数 + invariant 扫描。
...
| 风险 | 评估 | 缓解 |
| --- | --- | --- |
| A 既不答 B 也不问人（直接产 output）导致 to-agent 永久 awaiting_human | 中：A 忽略 Clarify Request | A6 fail `clarify-to-agent-timeout-no-answer` + retries 兜底 |
| A 一次重跑同时吐 answer + clarify 路径分裂 | 中：时序复杂 | A5 v1 互斥 fail，强制分两次重跑 |
| A 升级问人期间 to-agent 误判完成 | 中：scheduler frontier 误推 | `awaiting_human` 显式不入 completed 集合（loadOpenClarify 扩展收集 to-agent session） |
| 多源多 to-agent 指向 A 的并发触发 race | 低：DB 单写者 + 多源汇总屏障 | 对齐 RFC-056 多源汇总 atomic 检查 |
| to-agent 复用 `awaiting_human` 与现有 lifecycle 语义混淆 | 低：to-agent park 用既有 `park-human`/`resume-clarify`，零新转移 | A12 措辞澄清复用 + 节点 kind 区分 to-agent 与 self-clarify |
