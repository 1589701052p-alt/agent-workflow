...
| D7  | 单 Agent 能力边界                        | **支持反问；「允许反问」开关默认开**；不做模型/运行时启动覆盖。反问语义=可问可不问（optional 指令，设计门 F12——现 clarify 边为强制先问，不采用）。~~开关放高级设置~~（**用户 2026-07-11 修订**：反问开关移出高级折叠，直接放任务内容区）。 |
| D8  | 临时空间产出交付                         | **复用现有 diff 视图**（产出=对空根提交的 diff）；目录保留在 scratch 根下可手动取用，GC 策略并入 worktree 策略。                                                                                                                                                     |
| D9  | 深链与旧路由（session 自定，用户未反对） | 资源页深链预填后自动落在第 2 步（可回退）；`/workflows/$id/launch` 重定向到向导；RFC-159 编辑定时配置由向导吸收；~~执行空间默认「远端仓库」~~（**用户 2026-07-11 修订**：默认「临时空间」，仍记住上次选择）；默认执行方式=单个 Agent（选项文案「Agent」，置于首位）。 |
