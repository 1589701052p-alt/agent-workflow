...
### 1.3.1 经验性探测：这是一整类 bug，最常见触发器比原始事故更普通

落 RFC 前对 agent × review × clarify 组合做了端到端探测（17 场景，真 `runTask` + 真 `submitClarifyAnswers` / `submitReviewDecision`，见 `packages/backend/tests/clarify-review-combination-scenarios.test.ts` + `fixtures/scenario-opencode.ts`）。结论：§1.3 的事故只是这一类 bug 的一个面，**最常见的触发器（S8）比它普通得多**。

**4 个确认的 RED（同一根因：cci 跨节点不同步）**：

| 场景 | 触发拓扑 | 严重度 | 危害 |
|---|---|---|---|
| **S8** | **`A(反问)→B→检视(B)`**——一个会反问的 agent + 一个不反问的中间 agent + 检视 | 高 | approve 后 Layer B 见 `B.cci=0 < A.cci=1` 误判 stale → 重跑 B（内容不变）→ **用户被迫重评刚批准的内容**。无需 iterate/reject，就是最普通的"先问清楚再交给下一个 agent"拓扑 |
| **S3** | review iterate + 期间 self-clarify（原始事故 `01KSHVXCH6RQ5F5P64MZ4FZVN6`） | 高 | 同上 |
| **S6** | review reject + 期间 self-clarify | 中 | 同上——证明 bug 类**蔓延到 reject 路径**，非 iterate 单点 |
| **S12** | diamond：`designer(反问)→检视分支` ∥ `designer→builder(不检视)分支` | 低 | 批准检视分支后**静默重跑**未变的 builder 分支（`builderDoneRows=2`）；非 readonly 还会无谓 churn 文件 |

**3 个 PASS 反向划清了精确触发边界**（也是 provenance 必须保持的回归锁）：

- **S10**（纯 agent 链 `A(问)→B→C`，无 review）PASS → cci 落差只在 **resume/re-entry 时咬人**，纯前向流不触发。
- **S11**（`A(问)→B(问)→检视`，中间 agent 也反问）PASS → 中间 agent 自己反问会把 cci 追平上游 → 不中招。**bug 严格需要"不反问的中间 agent"**。
- **S18**（`A(问)→revA` ∥ `B→revB`，单层直连）PASS → 检视直接接反问 agent（无中间节点）时 cci 对齐 → 不触发。

即：**bug = 「一个 cci 落后于其反问上游的节点，被（传递地）一个 review 消费，且在 resume 时被重算」**。provenance 下该节点的 `consumed[上游]` 指向它实际消费的上游行 → `isNodeRunFresh` 恒真 → 四个 RED 全部消失，三个边界 PASS 守恒。
...
**AC-12（组合场景矩阵）**：`clarify-review-combination-scenarios.test.ts` 的 4 个当前 RED 场景（S8 / S3 / S6 / S12，§1.3.1）在 provenance 后全部 un-skip 转绿；12 个当前 PASS 场景（S1/S2/S4/S5/S9/S10/S11/S13/S15/S16/S17/S18）保持绿（回归锁——证明 provenance 不破坏正确流程、且不改变 §1.3.1 划定的不触发边界）。S19（fanout）的 wrapper→agent→review 性质由 S13 结构性覆盖。
