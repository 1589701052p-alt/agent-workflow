  ...
  "runtimes": [
    {
      ...
      "version": "0.13.2",         // null = 未找到 / 不可执行
      "compatible": true,          // version 非 null 且 ≥ 协议门槛
      "minVersion": "0.13.0",      // 协议门槛（MIN_OPENCODE_VERSION / MIN_CLAUDE_CODE_VERSION）
...
- **D3 软硬语义（RFC-111 D10 的泛化）**：
  - 旧语义：opencode 缺失=红（硬依赖），claude 缺失=灰（可选运行时）。
  - 新语义按**默认运行时**泛化：`isDefault` 行缺失 → 红；非默认行缺失 → 灰；
    **任何行 incompatible（version 非 null 但低于门槛）→ 红**（装了但版本过低是
    可修的真实故障，与缺失的「可能从没打算装」不同）。
  - 开箱 opencode-only 机器：内置 claude-code 行 enabled 且缺失 → 灰点弱提示，
    不构成常驻红点（保 RFC-111 D10 的原始动机）。
...
- **D7 前端结构**：`describeRuntime` 重构为 `describeRuntimes(t, probe)` 纯函数
  ...
  **状态点 CSS 语义化改名**（Codex gate F3：现有变体按「原因」命名且颜色与新
  语义错位——`--missing` 是红、`--incompatible` 是灰，而新语义要求非默认缺失=灰、
  incompatible=红，按旧名直用必然错色）：`describeRuntimes` 对每 item 输出
  **severity**（`ok`=绿 / `fault`=红 / `soft`=灰 / `checking`=琥珀，原因→severity
  的映射即 D3），CSS 改为 `.homepage__runtime-dot--{ok|fault|soft|checking}` 四个
  语义变体（唯一消费方就是首页，旧四类同批删除，无迁移负担）；缺失非默认时
  文案套 `muted`。新增 CSS 仅限 severity 变体与 item 间距，不新造 chrome。
...
| `item.incompatible` | `{{name}} v{{version}} < v{{minVersion}}` | `{{name}} v{{version}} < v{{minVersion}}` |
...
- 某二进制 `--version` 挂死 → 该行在 D5 超时（默认 5s）后被 SIGKILL 回收并按
  `version: null` 处理（无子进程泄漏、无二次悬挂），端点整体仍在超时窗口内
  返回，其余行不受影响。
...
3. 注册自定义 fork（binaryPath 指向测试 fixture 假脚本，echo 一个版本号）→
   该行 `binary` = fixture 路径、`version` = 假脚本输出（binaryPath 感知锁定；
   fixture 手法沿 `runtime-routes.test.ts` 既有做法）。
4. binaryPath 指向不存在路径 → `version: null`、`compatible: false`，端点仍 200。
...
1. `describeRuntimes` 纯函数单测：双绿 / 非默认缺失（soft 灰）/ 默认缺失
   （fault 红）/ incompatible（fault 红，含非默认行——severity 映射即 D3/D7，
   gate F3 的错色回归靠这里锁）/ >3 收敛 + **最坏 severity 点名**（soft 行在前、
   fault 行在后 → 点名 fault 行；gate F5 回归）/ 空列表 noneEnabled /
   loading→checking。
