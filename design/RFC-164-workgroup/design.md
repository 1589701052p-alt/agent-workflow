...
| leader 跨轮上下文 | **session 续接优先**（RFC-026 先例：`node_runs.opencodeSessionId` schema.ts:728、opencode `--session` spawn.ts:49-51、claude `--resume` claudeCode/spawn.ts）+ **水位线增量注入**；无 session 时全量重注入 | 每轮上下文块设计为自足（全量态可重建），session 只是省 token 的优化，daemon 重启/换 runtime 不致断链 |
| 确认门状态 | **复用 `awaiting_review` 状态值**（task+node_run 双枚举已有，schema.ts:488/678），但**不复用** review.ts 的 doc_versions/决策机（无文档语义），新轻量端点 | 泊住-冒泡-resume-收件箱计数全链路现成（scheduler.ts:508-510） |
...
`runTask`（scheduler.ts:300 附近）分流：`task.workgroup_id` 非空 ⇒ `runWorkgroupTask(state)`，
...
| 两者 | 消息唤醒轮 | `direct_messages` 开 且 成员有未消费 @ 消息 且 该成员无 in-flight run（§6.3） |
...
- `workgroup_config_json.leaderContextWatermark`：记录 leader 已消费的最大 message id 与
  assignment 更新游标；**有 session** ⇒ 注入水位线之后的增量；**无 session**（首轮/捕获失败/
  runtime 更换/重启后续接失败）⇒ 注入全量态。上下文块自足性要求见 §6.1。
...
`direct_messages` 开时，@ 消息投给**空闲**成员 ⇒ mint 无单 run（§4.1），prompt=「你收到组内
消息，可回复/记录，不领任务」；其 run 计入 fc 硬顶计数。同成员有 in-flight run 则消息并入
该 run 结束后的下一次注入（不打断运行中进程——回合制边界投递，业界常驻系统的即时打断在本平台
统一退化为此语义）。
...
leader `decision=done`（fc：清单收敛）且 `completionGate` 开 ⇒ task → `awaiting_review`
（状态值复用，schema.ts:488；不触 review.ts 决策机/doc_versions）＋房间系统卡。
`confirm` 端点（任务成员均可，作答权同边界）：approve ⇒ done；reject（必带 comment）⇒
系统消息 + lw 唤醒 leader 新一轮（驳回意见作为高优注入项）/ fc 全员消息唤醒。驳回不回滚
worktree（组内继续改）。
...
| daemon 重启 | interrupted → autoResume → 回合循环按表重建（§4.3） |
| 消息风暴（agent 刷消息） | 每 run 消息条数上限（协议内声明+平台截断落系统告警）；黑板注入恒预算裁剪（§6.2） |
...
- 触顶 failed + 系统消息；resume 重入幂等（CAS 双驱动防护）；
...
- migration NNNN（journal +1）：五个 CREATE（workgroups/members/assignments/messages + builtin
  seed INSERT）+ 两个 ALTER tasks ADD COLUMN + 索引；多语句 `--> statement-breakpoint` 分隔
  （手写迁移既有教训）。
