...
| **PR-D'** ✅ | 步骤0 迁移垫片(`8851b5d`)+ 步骤1 T8 flag 停读(`c766cbf`)+ 步骤2 T4 停写 consumed_by + `openImmediateRounds` 派生化(`ff62df5`)| 已交付,CI 绿(T7 移 PR-E2,见下注) |
| **PR-E1** | 步骤3a:删 E 组 8 死注入器(`buildPromptContext`/`selectAnsweredRoundsForConsumer`/`buildClarifyNodeQueueContext`/`buildExternalFeedbackContext`/`buildNodeQueueExternalFeedback`/`buildQuestionerCrossClarifyContext`/`buildClarifyPromptContext`/`db_selectAnsweredSessionsForRerun`)+ `resolveTriggerForEntry`(PR-C 后死)+ 死注入器测试(rfc128-p5-a #1 等)| 🟡 中(纯死代码删) |
| **PR-E2 + T7** | 步骤3b:删 legacy mint(`submit*`/`trigger*`)+ immediate 账本 + `buildBorrowedAgent` + **T7**(`hasPersistentStop`/`latestPersistentStopAt` 删,`resolveCrossNodeStopped` 收敛读 questioner 节点 node 级 directive,node last-write-wins 等价 RFC-123 recency)+ 迁移垫片(遗留 cross stop 补 node 级 Q stop)+ ~9 hasPersistentStop + ~26 legacy-mint 测试(与 `submitCrossClarifyAnswers` 深度耦合,一起删/迁移)| 🔴 高(borrow ledger + RFC-123 recency + 大测试迁移) |
| **PR-F** | 步骤4 drop-column(**先 DROP INDEX 再 DROP COLUMN** 3 表 consumed_by + `clarify_rounds.directive` + `deferred_question_dispatch`;forward-only)| 🟡 中(不可回退,journal 72→73) |

**⚠️ T7 归入 PR-E2（2026-07-02 排序发现）**：T7(`hasPersistentStop` 收敛节点状态)的收敛代码简单(resolveCrossNodeStopped 读 questioner node 级 directive,node last-write-wins 天然等价 RFC-123 recency gate),但触发 ~9 个测试的 `hasPersistentStop` import,而这 8/9 个测试**同时用死代码 `submitCrossClarifyAnswers`**(步骤3b/PR-E2 删)——两者测试深度耦合。单独 T7 会留这些测试半红(hasPersistentStop 删但 submitCrossClarifyAnswers 还在),故 T7 并入 PR-E2 一起删/迁移这些 legacy cross 路径测试。crossClarifySessions.directive 列保留作审计(design §1)。
