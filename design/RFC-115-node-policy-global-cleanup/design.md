...
### 实现 gate
- **PR-A（adversarial-review，base HEAD~1，verdict needs-attention）— 1 finding [high] 已 fold**：旧 RFC-042 后端测试仍锁死 per-node retries 语义——`scheduler-envelope-followup-integration.test.ts` 用 `node.retries:0/1/3` 驱动重试、`envelope-followup-source-grep.test.ts` 锁 `pickNumber(node,'retries') ?? 3`；RFC-115 把预算切到 `opts.defaultNodeRetries ?? 3` 后这些 stale→红（全 backend 9 fail 中的 5 个）。Fold：4 个集成测试改用 `runTask({ defaultNodeRetries })` 驱动（保留显式 `defaultNodeRetries:0` no-retry 路径），grep guard 改锁 `opts.defaultNodeRetries ?? 3` + 断言 per-node lookup 已删。修后这俩文件 6 pass；CI 进一步暴露**另 8 个**同类 node.retries 测试（scheduler/scheduler-audit-s02/scheduler-port-validation/rfc097-*/scheduler-boundary-presnapshot/rfc092 + shorthand `{ retries }` 漏网的 scheduler-default-retries），用 subagent 批量 + 手补全迁 `runTask({ defaultNodeRetries })`（CI Received:4 → 9→6→3→0）。
- **PR-C（adversarial-review，base 835789c，verdict approve）— 零 findings**：DROP agents 5 列 + 0057 pre-drop 守卫 + 契约收口 + agent-md 路由全部通过审视。
- **PR-D/E/B（adversarial-review，base a227b66，verdict needs-attention）— 1 finding [high] 已 fold**：PR-D 删 config 6 死字段后，跨 RFC-113 跳级升级时 `loadConfig` 的 zod **strip 静默丢** config 生成默认值，**且 0057 agents 守卫不覆盖 config-only 场景**（与 PR-C 不对称、且下次 config save 永久删 disk key）。Fold：新增 `assertConfigDefaultsMigrated`（runtimeRegistry.ts）对称 fail-loud 守卫——读 **raw** config.json（绕过 zod strip）检测 6 legacy 字段，若存在但内置 runtime profile 全 NULL（RFC-113 config→runtime backfill 没跑）则 boot **ABORT**；接进 start.ts **try/catch 之外**（否则 warn-and-continue 吞掉 throw）。`rfc115-config-defaults-guard.test.ts` 5 pass。PR-E migration 0058 + PR-B agents 运行时列无 material blocker。
