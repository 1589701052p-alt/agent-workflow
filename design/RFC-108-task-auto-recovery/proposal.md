...
**配置接线（D2）**
- AC-1：default 配置（无 per-node `timeoutMs` override）启动的节点，`runNode` 收到的 `timeoutMs === defaultPerNodeTimeoutMs`（30min）；超时触发 SIGTERM→SIGKILL 并判 `node-timeout`。floor 由共享 `services/launchRuntimeConfig.resolveLaunchRuntimeConfig` 解析，**透传到全部 `StartTaskDeps` 构造点**：tasks 路由 start/multipart×2/resume/retry + **repair-options/repair**（`applyRepairOption`→`resumeAfterApply`→`resumeTask`，Codex 设计 gate P2）+ **parked clarify/review resume**（`clarify.ts`/`reviews.ts`）+ **fusion 引擎内部 startTask**（`FusionDeps.defaultPerNodeTimeoutMs`）——Codex 实现 gate P2 补全。（launch-path 单测 + 源码层文本断言锁全站点）
- AC-2（PR-B 范围调整——Codex 实现 gate P1）：per-task 预算（duration/token）的**自动从 config 默认接线移出 PR-B**，不做。原因：config loader **无版本迁移**（只 backfill 默认），存量 config 文件已把旧的 `defaultPerTaskMaxDurationMs: 1h` 持久化进磁盘，一旦消费会把它当硬上限、`limits.ts` ticker 取消任务，而 `canceled` **非 resumable** → 不可恢复地误杀合法长任务。per-node 30min floor（AC-1）已兜住 hung 子进程的成本/挂死（且 failed 节点可 retry）；per-task 预算要安全默认需配套 config-migration 框架，推后续 RFC。`input.maxDurationMs`/`maxTotalTokens` **显式设值仍照常生效**（行为零变更）；config 的 `defaultPerTaskMax*` 字段保持原状（仍为 advisory、未自动消费）。
