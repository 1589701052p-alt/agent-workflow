...
**T4（AR-01）默认 per-node 硬超时下限（D2：30min 可配）**
- `routes/tasks.ts` `resolveLaunchRuntimeConfig`（现仅返回 `{commitPush?, maxConcurrentNodes?}`，`:150-167`）增读 `cfg.defaultPerNodeTimeoutMs`，经 `StartTaskDeps` 透传。
- **透传必须覆盖全部 `StartTaskDeps` 构造点，而非只 3 个 HTTP 入口**（Codex 设计 gate P2）：除 `startTask`/`resumeTask`/`retryNode`，还有**第 4 个** deps 构造点——RFC-057 修复路径：`applyRepairOption` 的 `resumeAfterApply`（如 auto-repair 首发的 `S4.kick-task`）用 **repair `deps: StartTaskDeps`** 调 `resumeTask`（`lifecycleRepair.ts:293-308`）。若漏接，手动/自动修复 kick 的 scheduler 仍跑无硬超时节点 → 破 AC-1 的自动半。故在 `runtimeConfigOpts` 单源构造，所有 deps 站点（含 repair 路由 + auto-repair loop T19）共享同一 floor。
- scheduler 现 `nodeTimeoutMs = pickNumber(node,'timeoutMs') ?? opts.defaultPerNodeTimeoutMs`（`scheduler.ts:1697`）——dep 接上后 floor 自动生效；per-node override 仍可调高。
- 不影响 awaiting_*（停在 `awaiting_human`，非 `runNode` 下的 running）。
- 测试：default launch 的节点在 `runNode` 收到 `timeoutMs===default`；超时走 SIGTERM→SIGKILL 判 `node-timeout`。

**T5（AR-02）per-task 预算机制接线（默认不限）**
- `StartTaskDeps` 加 `defaultPerTaskMaxDurationMs?` / `defaultPerTaskMaxTotalTokens?`；`resolveLaunchRuntimeConfig` 读 config（**仅 >0 才返回**，0 视为「不设默认」）；`startTask` 插入改 `input.maxDurationMs ?? deps.defaultPerTaskMaxDurationMs ?? null`、token 同理。这使配置字段从「消费方为零的死值」变为真正生效（修 AR-02）。
- **shipped 默认 duration = 0（unlimited），实现期 D5 微调**（原 1h 死值改 0）：per-task 限额超限走 `limits.ts cancelTask`，而 `canceled` **不在 `resumeTask` 的 allowedFrom**（非 resumable），1h 硬取消会不可恢复误杀合法长跑多节点任务；hung 子进程的成本/挂死已由 T4 per-node 30min floor 兜住（且 failed 节点可 retry）。故 v1 只接通机制、不默认设 per-task cap。token 默认 0=unlimited（行为零变更）。
- `accumulateTokens` 嵌套 cache 漏计已由 RFC-103 T3 修复，token 限额此后才真实有效。
...
**T8（AR-16）S2 cross-clarify 误杀修复**
- `stuckTaskDetector.hasOpenClarifySession`（`:153-162`，只读 `clarify_sessions`）与 `options-S2.ts` `S2.reopen` preflight 改读统一 `clarify_rounds`（status='awaiting_human'，RFC-058 dual-write 自带 self+cross）。
- 回归：纯 cross-clarify awaiting_human >30min **不**产 S2 finding。
...
**T16（AR-20）廉价可观测补漏**
- `reapOrphanRuns`：被翻 interrupted 的 node_run 若 parent 是 awaiting_* → emit boot-time lifecycle_alert（今天静默解耦）。
...
- 前置：(a) 熔断未隔离（T11）；(b) cause 严格 `daemon-restart`；(c) `pre_snapshot` 可解析（否则落既有 snapshot-lost 升级）；(d) 非 awaiting_*（那是停在人）。
