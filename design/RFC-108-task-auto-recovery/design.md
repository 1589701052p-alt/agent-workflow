...
- scheduler `nodeTimeoutMs = pickNumber(node,'timeoutMs') ?? opts.defaultPerNodeTimeoutMs`（`scheduler.ts:1697`）——floor 自动生效，per-node override 仍可调高。不影响 awaiting_*（停在 `awaiting_human`，非 `runNode` 下 running）。
