...
**T10（AR-13）写时 U1 唯一约束** ⛔ 本会话**已尝试、回退、推后续**（设计级阻塞）

- 尝试：partial unique index `CREATE UNIQUE INDEX uq_node_runs_active_slot ON node_runs(task_id, node_id, review_iteration, COALESCE(shard_key,'')) WHERE status IN ('awaiting_review','awaiting_human')`（列名核对正确）。把全量 backend 当 shadow 验证。
- **发现（回退原因）**：enforce 后 **10 个既有测试红**，暴露**设计级冲突**——
  1. **U1 修复规则（RFC-057 `U1.cancel-older-keep-newest`/`cancel-newer-keep-oldest`）+ U1 扫描（RFC-053 checkU1）测试 全部靠「直接 insert 两条同 slot awaiting\_\* 行」构造违例**。写时唯一约束让违例**无法经普通 insert 构造** → 这两套测试 un-settable，且 U1 detect/repair 对**新数据彻底变 dead**（只剩 legacy 行用），等于**删掉一整条修复规则的语义**——非「快速 guard」，是 lifecycle-repair 面的实质变更。
  2. **cross-clarify 多轮**（`createCrossClarifySession` 同 (cc node, loopIter) 第二轮 mint）命中索引——需先确认 continue-轮是否在 mint 下一轮前真正 settle 上一轮 awaiting_human（若不 settle = 真双 park，则 U1 不变式本身对 cross-clarify 不成立）。
- **结论**：写时 U1 enforce 需要**专项**：① 决定 U1 scan/repair 在 enforce 后的去留/legacy 语义（连带改 ~7 测试）；② 回源核实 cross-clarify 多轮 settle 时序；③ 真正的 shadow 发布（非单 commit 能完成）。**已干净回退**（删 migration 0052 / schema 索引 / journal / 测试计数），树恢复绿。推独立后续（同 T6 GC-side 一道作 PR-C 收尾批）。
