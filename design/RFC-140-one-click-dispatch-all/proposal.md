...
2. **塌缩边界**：改派到第三节点 → 常规 override（golden-lock 逐字不变）；questioner 行已
   ...
   建（seal 戳归一化）；**designer 行已存在且带第三节点 override → 塌缩时 override 归一化清
   空**（effective 回落设计节点，Codex 设计门 P2）；scope 表双写 lockstep（clarify_rounds +
...
4. **越权防护**：仅 staged 未点发（`auto_dispatch_deferred_at` 为空）的条目在续跑 done 后
   **不被**自动下发；**deferred 条目被 unstage（撤回待下发）后登记随之清除、不再自动发，
   re-stage 也不复活登记**（须重新点批量下发——Codex 设计门 P1）。
5. **重启韧性**：deferred 登记落库；daemon 重启 + 任务 revival 后 tick 照常自动补发。
6. **补发失败自愈**：自动补发撞**可恢复** Conflict（in-flight / target-changed /
   designer-not-ready，复用既有 `DESIGNER_DEFERRABLE_CONFLICTS` 白名单）→ 静默留待下一 tick，
   不失败任务、不丢登记；**白名单外的不可恢复 Conflict → 清登记 + WARN 回手动轨道**，绝不
   静默空转（Codex 设计门 P2）。
