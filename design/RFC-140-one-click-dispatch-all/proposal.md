...
4. **越权防护**：仅 staged 未点发（`auto_dispatch_deferred_at` 为空）的条目在续跑 done 后
   **不被**自动下发；**deferred 条目被 unstage（撤回待下发）后登记随之清除、不再自动发，
   re-stage 也不复活登记**（stage 与 unstage 双向清列 + stage/unstage 纳入 question-write
   lock 与 dispatch 串行，消除晚盖登记竞态——Codex 设计门 P1 + 二轮 P2）。
5. **重启韧性**：deferred 登记落库；daemon 重启 + 任务 revival 后 tick 照常自动补发。
6. **补发失败自愈**：补发按 **effective target 逐 home 隔离**；可恢复 Conflict（in-flight /
   target-changed / designer-not-ready，复用既有 `DESIGNER_DEFERRABLE_CONFLICTS` 白名单）→
   该 home 静默留待下一 tick，不失败任务、不丢登记；**白名单外的不可恢复 Conflict → 仅清该
   home 的登记 + WARN 回手动轨道**，其他 home 照常自动发，绝不静默空转、绝不连坐（Codex 设
   计门 P2 两轮）。
