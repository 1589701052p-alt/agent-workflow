...
- **scope**：self（`submitClarifyAnswers`）+ questioner（`submitCrossClarifyAnswers`）三步**全落**（含 session CAS + consume 整轮 + 同步 tx flip）。cross 的续跑 mint 按 finding 2 留 tx 后（races 已封）。
