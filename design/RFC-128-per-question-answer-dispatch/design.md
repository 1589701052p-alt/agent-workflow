...
- **scope（本轮 vs 跟进）**：self 路径（`submitClarifyAnswers`）三步全落。**questioner 路径（`submitCrossClarifyAnswers`）本轮只落第 1 步**（any-dispatched guard，关其顺序数据丢失）；第 2+3 步（consume + 同步 dbTxSync）列**跟进**——其临界区还串着 designer / deferred 分支，原子化值得单独专审 + adversarial gate。在跟进前 questioner 路径仅余**并发 race 窗口**（顺序数据丢失已挡）。
