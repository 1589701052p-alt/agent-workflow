...
- **集中回答界面**（前端，净新）：任务详情新 pane，`listTaskQuestions` 取 `answerSummary===null` 待答题、按 `originNodeRunId` 分组平铺多个 `QuestionForm` 块；逐题草稿（已支持跨轮）+ 逐题/批量「确定」调上面端点（`defer=true`）；scope 仅 cross 轮渲染、directive 按轮呈现；改派下拉 `ClarifyQuestionHandler`（designer 条目答后可用）。复用 `QuestionForm`/`Select`/`Dialog`/`Card`/`StatusChip`/`EmptyState`/`ErrorBanner`/`LoadingState`，无原生 chrome。

## 11. 待下发 gate（D5）

`stageTaskQuestion`（`taskQuestions.ts:790-805`，现零答案校验）加：该题须已 seal（`sealed_at!=null` 或 `answerSummary!=null`）否则 4xx；前端隐藏未答题的 stage 按钮（`TaskQuestionList.tsx:344` 的 `hasStage` 加 `answered` 条件）。
