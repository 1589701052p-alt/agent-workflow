...
>
> **Codex 设计 gate 第二轮 R2-1（live state 回退）**：上「合并消除 stranded」是**代码级**；**一旦 P5-BC 在生产产生 self/q dispatch 状态（`dispatched_at` 戳 / `trigger_run_id` 绑 / partial in-flight），代码-only 回退仍 strand 它**（旧代码无 self/q park/dispatch/per-entry consume path、P5-0 只拒未来不清既有）。**P5-BC 回退点措辞从「可单独回退」改为「代码可回退；live self/q dispatch 状态须先 drain（或迁移）否则不可无数据回退」**——安全回退须先 **drain-then-rollback**（禁写新 self/q dispatch → 等所有 in-flight self/q dispatched 跑到 consumed〔done+output，判据同 `isDispatchedEntryConsumed`，`taskQuestionDispatch.ts:566`〕→ 队列清空再回退）或 **migrate-then-rollback**（清 self/q `dispatched_at`/`trigger_run_id` + 轮重置整轮戳）。详见 `design.md §5.2.9` rollback playbook。
...
- **P5-BC-T4（dispatch 放开 + 五契约〔三 + 第二轮 R2-2/R2-3〕，原 P5-C-T1 + F2/F3/F4 + 第二轮新增）**：`dispatchTaskQuestions`（`taskQuestionDispatch.ts:198`）去 designer-only（`:240` / `:258`），放开 deferred self/q 批量下发；**带五契约**：
  ...
  - **(e) route auto-split 推进契约（第二轮 R2-3，`design.md §5.2.13` + §11.1）**：§11.1 删了 per-card checkbox + 「批量下发=全部 staged」，故同 home 混 cause 全 staged 时 route **不得整批 reject**（否则前端全量提交→全量 reject 死循环）。route 收「全部 staged」后对**同 home 混 cause**（仅此类）**自动分安全批**——每 home 一时刻只下发一个 cause 类（确定性序，self/q 先、designer 后），其余留 staged，**response 新增「已下发 / 延后（下一批）分组 + 原因」字段**；前端展示「待第一批续跑完成后可下发」，第一批 done+output 后再下发（届时 (d) in-flight gate 已放行）。**precondition 违例（unsealed〔a〕/ per-origin〔c〕/ per-home multi-borrow / not-ready）仍 fail-fast 整批 reject**；检查序：先 precondition 拒类、全通过才 auto-split。这是 (b) 串行化的**同批**物理实现。前端落点配合 §11.1 / P4。
