...
- `.github/workflows/ci.yml`：`build-binary` matrix 加 `windows-latest`（Windows leg 不装 native opencode——`version`/`doctor` smoke 不需）+ 新 `check-windows` job（typecheck/lint/format+RFC-144 平台层测试，不全量 bun test——等 PR-3 闭环）+ `.gitattributes` 强制 LF。
