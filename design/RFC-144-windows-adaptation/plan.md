...
## PR-2｜文件系统 + ACL（中低风险，可与 PR-1 并行）

- **T6** oracle：file:// 跨平台往返单测（`pathToFileURL`/`fileURLToPath` 在 `C:\…` 与 `/x/y` 下等价）；ACL 实证测试骨架（Windows 用 `icacls` dump 断言，先红）；symlink→junction 回归锚点。
- **T7** file:// 全局统一：`runner.ts:549,1752,1822,1824` + `pluginInstaller.ts:229-230` 的字符串拼接改 `pathToFileURL`/`fileURLToPath`；POSIX 行为等价，全平台统一删分支。
- **T8** symlink：`util/platform.ts` 加 `linkSkillDir(target,dst)`（POSIX `symlinkSync(dir)` / Windows `junction`，文件型降级 copy）；`runner.ts:1596` + `claudeCode/config.ts:60` 改调它。补「Windows 上 symlink-based repo 攻击行为等价」回归测试（`upload.ts` 的 `realpathInside`/`lstat` 不动）。
- **T9** ACL 闭合：新建 `util/fs-perms.ts` 的 `secureFile`/`secureDir`（POSIX chmod 600/700 / Windows `icacls /inheritance:r /grant:r "$USER:..."`）；`auth/secretBox.ts:24,33` + `auth/token.ts:29,39` + `pluginInstaller.ts:181` 改调。ACL 实证测试转绿（Windows `icacls` dump 断言仅当前用户，无 Everyone/Users）。
- **T10** 长路径：`util/platform.ts` 加 `toLongPath(p)`（`\\?\` 前缀）；`doctor` 加 LongPathsEnabled 检查（未启用标黄不阻断）；二进制 manifest 加 `longPathAware`。
- 验收：门禁全绿；file:// 往返 + ACL 实证 + symlink 回归绿；POSIX chmod/symlink 测试不回归。
...
## PR-4｜MCP + 备份 + indexers（降级路径，中低风险）

- **T18** oracle：MCP probe 在 Windows 探测一个真实 stdio server 的测试骨架；tar 备份 Windows 分支单测。
- **T19** MCP env：`mcpProbe.ts:367` 白名单加 Windows 键（`USERPROFILE`/`HOMEDRIVE`+`HOMEPATH`/`PATHEXT`/`SystemRoot`/`ComSpec`）；`util/platform.ts` 加 `normalizeEnvForPlatform(env)`（HOME→USERPROFILE 兼容注入）；`.cmd` shim 的 PATHEXT 解析用真实 MCP server 验证。
- **T20** tar 备份：`services/backup.ts:152` Windows 分支探测 `tar.exe`（Win10 1803+ 自带 bsdtar）；可用直接用，不可用降级 Node `zlib`+`tar`(npm) 或 `Compress-Archive`（`.zip`，文档标注）。
- **T21** SCIP indexers：不改代码；文档列明六个 indexer 在 Windows 的可得性（scip-typescript/python/go 多数有 Win 构建、rust-analyzer 有、scip-clang 可能无、scip-java 有）；`probeIndexer` 的「缺失即降级」路径加 Windows 单测。
- 验收：门禁全绿；MCP probe + tar 备份 Windows 测试绿；indexer 降级路径单测绿。
