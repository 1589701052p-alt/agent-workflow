...
**最近更新**：2026-05-15（RFC-003 落地 — Canvas 给 agent / wrapper-loop 节点加 catch-all 左侧 handle，drop 默认 `target.portName=source.portName`，EdgeInspector 支持改名 + 删除，NodeInspector 显示模板里没入边的 `{{x}}` 引用；v1 路线图保持 81/81）
